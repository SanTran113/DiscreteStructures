   ...
   private boolean moveDFS(Point pos, GridValues[][] grid, List<Point> path) {
      ...
      if (withinBounds(pos, grid) &&
         ...
         if (grid[pos.y][pos.x] == GridValues.GOAL) {
            ...
            grid[pos.y][pos.x] = GridValues.SEARCHED;
            found =  moveDFS(rightN, grid, path)
                    || moveDFS(leftN, grid, path)
                    || moveDFS(downN, grid, path)
                    || moveDFS(upN, grid, path);
         }
         if (found) {
            path.add(0, pos);
         }
...
//         found = true;
//      else
//         found = findDFS(searchVal, node.left) || findDFS(searchVal, node.right) || findDFS(searchVal, node.up) || findDFS(searchVal, node.down);
//
//      if (found)
