 ...
 */
public final class TransformedEntity implements ExecuteActivity, AnimatingEntity{
    private String id;
    ...
    public TransformedEntity(String id, Point position, List<PImage> images, double actionPeriod, double animationPeriod) {
        this.id = id;
...
//    }
    ...
    private boolean moveToTransformedEntity(WorldModel world, Entity target, EventScheduler scheduler) {
        if (Functions.adjacent(this.position, target.getPosition())) {
            world.removeEntity(scheduler, target);
            return true;
        } else {
            Point nextPos = nextPositionTransformedEntity(world, target.getPosition());
    ...
    public void executeActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Entity> transformedEntityyTarget = world.findNearest(this.position, new ArrayList<>(List.of(NewEntity.class)));

        if (transformedEntityyTarget.isPresent()) {
            Point tgtPos = transformedEntityyTarget.get().getPosition();

            if (this.moveToTransformedEntity(world, transformedEntityyTarget.get(), scheduler)) {
//                setActionPeriod(getActionPeriod() - 0.15);
...
//    }
    ...
    private Point nextPositionTransformedEntity(WorldModel world, Point destPos) {
